...
class MaskServiceClient:
    ...
    def __init__(self):
        ...
        self.mask_publisher = rospy.Publisher("/seg_image", RosImage, queue_size=5)
    ...
    def call_mask_service(self, rgb_image_path="", text_prompt=""):
        ...
        try:
            ...
            if response.mask.height == 0 or response.mask.width == 0:
                ...
                return None
            bwimage = ros_numpy.numpify(deepcopy(response.mask))
            bwimage[bwimage>0] = 255
            clrimage = np.dstack((bwimage,bwimage,bwimage))
            # import pdb;pdb.set_trace()
            mask_msg = ros_numpy.msgify(RosImage, clrimage, encoding='rgb8')
            mask_msg.header.frame_id = "head_camera_rgb_optical_frame"
            mask_msg.header.stamp = rospy.Time.now()
            
            # mask_msg = ros_numpy.msgify(RosImage, maskimage, encoding='mono8')  # Assuming mask is single-channel (grayscale)
            # mask_msg.header.frame_id = "head_camera_rgb_optical_frame"
            # mask_msg.header.stamp = rospy.Time.now()
...
if __name__ == "__main__":
    try:
        ...
        if len(sys.argv) != 4:
            print(
                "Usage: python mask_service_client.py <image_path> <text_prompt> <output_mask_path>")
            sys.exit(1)
