...
def sanity_check(argv):
    ...
    if not os.path.isdir(input_dir):
        raise(f"Error: The directory '{input_dir}' does not exist or is not a valid directory.")
    ...
    if not image_files:
        raise(f"No image files found in the directory '{input_dir}'.")
    
    # Ensure text_prompt is provided
    if not text_prompt:
        raise("Error: 'text_prompt' is required but not provided.")
